		...
		super(database, tableName, options, (error) => {
			async.series([
				(callback) => {
					...
					var createTable = {
						$create: {
							$table: { $ine: this._schemaName + '.' + this._baseName }
						}
					};

					// check for additional create-options
					if (opt.$tableOptions){
						_.extend(createTable.$create, opt.$tableOptions);
						delete opt.$tableOptions;
					}
					createTable.$create.$define = opt;
