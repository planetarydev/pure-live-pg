...
class PGDatabase {
	constructor(options) {
		...
		// the insert update or delete is performed we have to wait for the reactivity is done
		this._runningDbActions = {};
	...
	_cacheQuery(query){
		...
		_.forEach(query._queryObject.tableDependencies, (tableName) => {
			...
			this._queriesByTable[tableName].push(query.queryId);
		});
	}
	...
	_unCacheQuery(query){
		...
		_.forEach(query._queryObject.tableDependencies, (tableName) => {
			...
			if (this._queriesByTable[tableName]){
				var index = this._queriesByTable[tableName].indexOf(query._queryId);
	...
	_onNotify(notification){
		...
		console.log(data.client_id + ' - ' + data.schema + '.' + data.table);
		console.log(this._queriesByTable);

		var relatedTable = data.schema + '.' + data.table;
		// get all queries for the related table
		var queryIds = this._queriesByTable[relatedTable];

		// check if there are currently reactive queries register
		// if not, still exit!
		if (!queryIds) {
			console.log('REACTIVE done no queryIDs !queryIds: ', data.client_id);
			delete this._runningDbActions[data.client_id];
			return;
		}
		if (queryIds.length == 0) {
			console.log('REACTIVE done no queryIDs: ', data.client_id);
			delete this._runningDbActions[data.client_id];
			return;
		}
		...
		var queryRunners = queryIds.map(id => {
			...
			return function(callback) {
				self._queryCache[queryId].reRun(callback);
			}
		});

		async.parallel(queryRunners, (error) => {
			console.log('REACTIVE done: ', data.client_id);
			delete this._runningDbActions[data.client_id];
		});
	}
