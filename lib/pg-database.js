...
class PGDatabase {
	...
	_initNotifyTrigger(tableName, callback) {
		...
		let sql = this._sql.createNotifyTrigger
	...
	_initReactivePg(callback){
		...
		let sql = this._sql.createTableReactivity
			.split('@@ownSchema').join(this._ownSchema)
	...
	_initNotifyFunction(callback) {
		/*var sql = `
			...
			$$ LANGUAGE plpgsql;`;*/
		let sql = this._sql.createTriggerFunctionNotifyChanges
			.split('@@ownSchema').join(this._ownSchema)
	...
	_onNotify(notification){
		...
		async.each(firstQueries, (q, callback) => {
			...
			for(var i=0, max=secondQueries.length; i<max; i++){
				...
				this._queryCache[qid]._onReactivity.apply(this._queryCache[q.queryId], [false /*another client has made changes*/, data]); // XXX reactivity-approach
		...
		async.series([
			...
			(callback) => {
				...
				this.getConnection((error, connection) => {
					...
					let sql = this._sql.createOwnSchema.split('@@ownSchema').join(this._ownSchema);
					this.query(connection, sql, [], (error, result) => {
						this.releaseConnection(connection);
	...
	query(connection, sql, values, callback) {
		...
		connection.query(sql, values, (error, result)=>{
			if (error) console.log('onQuery ERROR:', error, sql, values);
			return callback(error, result);
		});
	}
}
