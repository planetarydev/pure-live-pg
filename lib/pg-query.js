	...
	run(callback){
		...
		query = {
			$select: {
				...
				$where: this._queryObject.selector // fixed empty $where clause -->Object.keys(this._queryObject.selector).length > 0 ? this._queryObject.selector : null // can't pass an empty object
			}
		};
