...
class PGReactiveQuery extends PGQuery {
	...
	run(callback) {
		...
		if (this._dataLoading.type === 'paging' || this._dataLoading.type === 'incremental'){
			...
			if (this._dataLoading.type === 'paging'){
				...
				this._dataLoading.currentPage = 0;
			}
	...
	gotoPage(pageNo, callback){
		...
		if (pageNo * this._dataLoading.limit > this._dataLoading.totalRowCount)
			return callback();
		...
		this._dataLoading.currentOffset = pageNo * this._dataLoading.limit;
