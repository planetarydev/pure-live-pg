...
class PGReactiveQuery extends PGQuery {
		...
		async.series([
			function(callback){
				self._database.getConnection(function(error, connection){
					...
					connection.query('DELETE FROM core_reactive WHERE query_id = $1', [self.queryId], function(error, result){
			...
			function(callback){
				...
				self._database._unCacheQuery(self);
	...
	run(callback) {
		...
		this.executeQuery(false/*rerun*/, callback);
	}
