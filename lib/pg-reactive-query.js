...
class PGReactiveQuery extends PGQuery {
	...
	get isStaled() {
		return (this._database._staledQueries.indexOf(this._queryId) > -1);
	}
	...
	_onReactivity(changesMadeByOwnQuery, data, callback) {
		...
		//console.log('_onReactivity', changesMadeByOwnQuery, reactivityLevel, data.action);
		...
		// changes happend are made by another client
		if (reactivityLevel == 'own' && !changesMadeByOwnQuery) {
			return callback();
		}

		// if the reactivityLevel is set to "own" or "always" and the changes are made by own query
		// we will directly reRun the query and publish the updated results
		if ((reactivityLevel == 'own' || reactivityLevel == 'always') && changesMadeByOwnQuery) {
